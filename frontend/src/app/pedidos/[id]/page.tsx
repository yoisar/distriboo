          ...
          <>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 mb-6 shadow-sm">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Pedido #{pedido.id}</h2>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium text-gray-800 dark:text-gray-200">{pedido.cliente?.razon_social || "-"}</p>
                </div>
                <div>
                  <span className="text-gray-500 dark:text-gray-400">Provincia</span>
                  <p className="font-medium text-gray-800 dark:text-gray-200">{pedido.cliente?.provincia?.nombre || "-"}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium text-gray-800 dark:text-gray-200">{pedido.observaciones || "-"}</p>
                </div>
              </div>
            </div>
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {pedido.detalles?.map((d) => (
                    <tr key={d.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      <td className="px-4 py-3 text-sm font-medium text-gray-800 dark:text-gray-200">{d.producto?.nombre || `Producto #${d.producto_id}`}</td>
                      <td className="px-4 py-3 text-sm text-right text-gray-700 dark:text-gray-300">${Number(d.precio_unitario).toLocaleString("es-AR")}</td>
                      <td className="px-4 py-3 text-sm text-right text-gray-700 dark:text-gray-300">{d.cantidad}</td>
                      <td className="px-4 py-3 text-sm text-right font-medium text-gray-900 dark:text-gray-100">${Number(d.subtotal).toLocaleString("es-AR")}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-800 dark:text-gray-200">${Number(pedido.subtotal).toLocaleString("es-AR")}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500 dark:text-gray-400">Costo logístico</span>
                  <span className="text-gray-800 dark:text-gray-200">${Number(pedido.costo_logistico).toLocaleString("es-AR")}</span>
                </div>
                <div className="flex justify-between text-lg font-bold border-t border-gray-200 dark:border-gray-700 pt-2">
                  <span className="text-gray-900 dark:text-gray-100">Total</span>
                  <span className="text-gray-900 dark:text-gray-100">${Number(pedido.total).toLocaleString("es-AR")}</span>
                </div>
              </div>
            </div>
